...
complex_t kappa_3d_3d_singular_integrand_outer(const complex_t alpha, const complex_t beta, const complex_t gamma, void *args_){
    ...
    real_t R_m, R_p;
    vector_t<real_t> dist=b_n.e[0]+b_n.e[1]/3.0+b_n.e[2]/3.0;
    ...
    real_t factor=b_n.nA*(b_n.r_m+b_n.e[0]/3.0+b_n.e[1]/3.0+b_n.e[2]/3.0);
    I_m = +1.0*(exp(-j*k*R_m)/R_m)*factor;
    I_p = -1.0*(exp(-j*k*R_p)/R_p)*factor;
    return 2.0*b_m.A*b_n.A*(I_m/b_n.V_m+I_p/b_n.V_p)/(4.0*pi);
}
